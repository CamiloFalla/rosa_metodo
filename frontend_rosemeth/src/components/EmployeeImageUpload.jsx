...
export default function EmployeeImageUpload({ employee, onImageUploaded }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:12348/empleados/${employee.id_empleado}/upload`, {
        method: 'POST',
        body: formData
      });
      ...
      // Llamar a onImageUploaded para que el padre (EmployeeDetails) actualice la vista
